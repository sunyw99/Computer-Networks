// TODO: copy your code from part 3.1 and implement part 3.3
...
void writeLog(struct SRHeader *header, const char *log_file) {
    ...
    else if (header->flag == 4) strcpy(log, "NACK ");
        ...
        if (recv_len > 0) {
            ...
            if (crc32(rcv_pkt.payload, rcv_pkt.header->len) == rcv_pkt.header->crc) {
                if (rcv_pkt.header->flag == 0) {
                    ...
                    if (rcv_pkt.header->seq < win_start_sequence) {
                        ...
                        for (i = 0; i < window_size; i++) {
                            if (window[i].header == NULL && win_start_sequence + i > NACK_sequence) {
                                old_NACK_sequence = NACK_sequence;
                                NACK_sequence = win_start_sequence + i;
                                break;
                            }
                        }
                        for (j = i; j < window_size; j++) {
                            if (window[j].header != NULL) {
                                FLAG_NACK = 1;
                                ack_header->flag = 4;
                                ack_header->seq = NACK_sequence;
                                ack_header->len = 0;
                                ack_header->crc = crc32(NULL, 0);
                                ack_pkt.header = ack_header;
                                bzero(ack_pkt.payload, sizeof(ack_pkt.payload));
                        ...
                        if (FLAG_NACK == 0) {
                            NACK_sequence = old_NACK_sequence;
                            ack_header->flag = 3;
                            ack_header->seq = win_start_sequence;
                            ack_header->len = 0;
                            ack_header->crc = crc32(NULL, 0);
...
int sender(const char *receiver_ip, int receiver_port, int sender_port, int window_size, const char *file_to_send, const char *log_file) {
    ...
    while (1) {
        while(sequence < win_start_sequence + window_size && (file_block_length = fread(send_pkt.payload, sizeof(char), MAX_PAYLOAD_SIZE, fp)) > 0) {
            ...
            if (FLAG_NACK == 1) {
                fseek(fp, MAX_PAYLOAD_SIZE * (win_start_sequence + window_size - NACK_sequence - 2) + last_fileblock, SEEK_CUR);
                FLAG_NACK = 0;
        ...
        while (1) {
            ...
            if (ack_len > 0) {
                ...
                else if (ack_pkt.header->flag == 4) {
                    //printf("NACK received. Resend.\n");
                    NACK_sequence = ack_pkt.header->seq;
                    fseek(fp, - MAX_PAYLOAD_SIZE * (sequence - NACK_sequence - 1) - last_fileblock, SEEK_CUR);
